 ...
 *  OnBoardHeader.h
 *
 *  assembled from Palm SDK 4.0 header files.
 *
...
/*
These are here if you want to use the old types
...
enum controlStyles {buttonCtl, pushButtonCtl, checkboxCtl, popupTriggerCtl, selectorTriggerCtl, repeatingButtonCtl}; 
...
void* FrmGetObjectPtr(const FormType *formP, UInt16 objIndex) SYS_TRAP(sysTrapFrmGetObjectPtr); 
...
UInt16 FrmGetControlGroupSelection(const FormType *formP, UInt8 groupNum) SYS_TRAP(sysTrapFrmGetControlGroupSelection); 
...
UInt16 DmNumDatabases(UInt16 cardNo) SYS_TRAP(sysTrapDmNumDatabases); 
...
UInt16 DmNumResources(DmOpenRef dbP) SYS_TRAP(sysTrapDmNumResources); 
...
Err DmRemoveRecord(DmOpenRef dbP, UInt16 index) SYS_TRAP(sysTrapDmRemoveRecord); 
UInt16 DmNumRecords(DmOpenRef dbP) SYS_TRAP(sysTrapDmNumRecords); 
MemHandle DmNewRecord(DmOpenRef dbP, UInt16 *atP, UInt32 size) SYS_TRAP(sysTrapDmNewRecord); 
MemHandle DmQueryRecord(DmOpenRef dbP, UInt16 index) SYS_TRAP(sysTrapDmQueryRecord); 
MemHandle DmResizeRecord(DmOpenRef dbP, UInt16 index, UInt32 newSize) SYS_TRAP(sysTrapDmResizeRecord); 
MemHandle DmGetResource(DmResType type, DmResID resID) SYS_TRAP(sysTrapDmGetResource); 
MemHandle DmNewResource(DmOpenRef dbP, DmResType resType, DmResID resID, UInt32 size) SYS_TRAP(sysTrapDmNewResource); 
...
Err DmGetNextDatabaseByTypeCreator(Boolean newSearch, DmSearchStatePtr stateInfoP,  UInt32 type, UInt32 creator, Boolean onlyLatestVers, UInt16 *cardNoP, LocalID *dbIDP) SYS_TRAP(sysTrapDmGetNextDatabaseByTypeCreator); 
...
Err DmDatabaseSize(UInt16 cardNo, LocalID dbID, UInt32 *numRecordsP, UInt32 *totalBytesP, UInt32 *dataBytesP) SYS_TRAP(sysTrapDmDatabaseSize); 
...
UInt32 MemHandleSize(MemHandle h) SYS_TRAP(sysTrapMemHandleSize); 
...
WinHandle WinCreateWindow(const RectangleType *bounds, FrameType frame, Boolean modal, Boolean focusable, UInt16 *error) SYS_TRAP(sysTrapWinCreateWindow);
...
Err BmpCompress(BitmapType * bitmapP, BitmapCompressionType compType) SYS_TRAP(sysTrapBmpCompress);
void* BmpGetBits(BitmapType * bitmapP) SYS_TRAP(sysTrapBmpGetBits);
ColorTableType* BmpGetColortable (BitmapType * bitmapP) SYS_TRAP(sysTrapBmpGetColortable);
UInt16 BmpSize(const BitmapType * bitmapP) SYS_TRAP(sysTrapBmpSize);
UInt16 BmpBitsSize(const BitmapType * bitmapP) SYS_TRAP(sysTrapBmpBitsSize);
void BmpGetSizes(const BitmapType * bitmapP, UInt32 * dataSizeP, UInt32 * headerSizeP) SYS_TRAP(sysTrapBmpGetSizes);
UInt16 BmpColortableSize(const BitmapType * bitmapP) SYS_TRAP(sysTrapBmpColortableSize);
void BmpGetDimensions(const BitmapType * bitmapP, Coord * widthP, Coord * heightP, UInt16 * rowBytesP) SYS_TRAP(sysTrapBmpGetDimensions);
UInt8 BmpGetBitDepth(const BitmapType * bitmapP) SYS_TRAP(sysTrapBmpGetBitDepth);
BitmapType* BmpGetNextBitmap(BitmapType * bitmapP) SYS_TRAP(sysTrapBmpGetNextBitmap);
...
char * StrCat(char *dst, const char *src) SYS_TRAP(sysTrapStrCat);		
char * StrNCat(char *dst, const char *src, Int16 n) SYS_TRAP(sysTrapStrNCat);						
Int16 StrLen(const char *src) SYS_TRAP(sysTrapStrLen);					
Int16 StrCompareAscii(const char *s1, const char *s2) SYS_TRAP(sysTrapStrCompareAscii);
Int16 StrCompare(const char *s1, const char *s2) SYS_TRAP(sysTrapStrCompare);
Int16 StrNCompareAscii(const char *s1, const char *s2, Int32 n) SYS_TRAP(sysTrapStrNCompareAscii);
Int16 StrNCompare(const char *s1, const char *s2, Int32 n) SYS_TRAP(sysTrapStrNCompare);
Int16 StrCaselessCompare(const char *s1, const char *s2) SYS_TRAP(sysTrapStrCaselessCompare);
Int16 StrNCaselessCompare(const char *s1, const char *s2, Int32 n) SYS_TRAP(sysTrapStrNCaselessCompare);
char * StrToLower(char *dst, const char *src) SYS_TRAP(sysTrapStrToLower);
char * StrIToA(char *s, Int32 i) SYS_TRAP(sysTrapStrIToA);
char * StrIToH(char *s, UInt32 i) SYS_TRAP(sysTrapStrIToH);
char * StrLocalizeNumber(char *s, char thousandSeparator, char decimalSeparator) SYS_TRAP(sysTrapStrLocalizeNumber);
char * StrDelocalizeNumber(char *s, char thousandSeparator, char decimalSeparator) SYS_TRAP(sysTrapStrDelocalizeNumber);
char * StrChr(const char *str, WChar chr) SYS_TRAP(sysTrapStrChr);
char * StrStr(const char *str, const char *token) SYS_TRAP(sysTrapStrStr);
Int32 StrAToI(const char *str) SYS_TRAP(sysTrapStrAToI);					
Int16 StrPrintF(char *s, const char *formatStr, ...) SYS_TRAP(sysTrapStrPrintF);						
Int16 StrVPrintF(char *s, const char *formatStr, char *arg) SYS_TRAP(sysTrapStrVPrintF);
...
char* FldGetTextPtr(const FieldType *fldP) SYS_TRAP(sysTrapFldGetTextPtr); 
...
// jwDone
FontID FntGetFont(void) SYS_TRAP(sysTrapFntGetFont);
FontID FntSetFont(FontID font) SYS_TRAP(sysTrapFntSetFont);
FontPtr FntGetFontPtr(void) SYS_TRAP(sysTrapFntGetFontPtr);
Int16 FntBaseLine(void) SYS_TRAP(sysTrapFntBaseLine);
Int16 FntCharHeight(void) SYS_TRAP(sysTrapFntCharHeight);
Int16 FntLineHeight(void) SYS_TRAP(sysTrapFntLineHeight);
Int16 FntAverageCharWidth(void) SYS_TRAP(sysTrapFntAverageCharWidth);
Int16 FntCharWidth(char ch) SYS_TRAP(sysTrapFntCharWidth);
Int16 FntWCharWidth(WChar iChar) SYS_TRAP(sysTrapFntWCharWidth);
Int16 FntCharsWidth(const char *chars, Int16 len) SYS_TRAP(sysTrapFntCharsWidth);
Int16 FntWidthToOffset(char*pChars, UInt16 length, Int16 pixelWidth, Boolean *leadingEdge, Int16 *truncWidth) SYS_TRAP(sysTrapFntWidthToOffset);
void FntCharsInWidth(const char *string, Int16 *stringWidthP, Int16 *stringLengthP, Boolean *fitWithinWidth) SYS_TRAP(sysTrapFntCharsInWidth);
Int16 FntDescenderHeight(void) SYS_TRAP(sysTrapFntDescenderHeight);
Int16 FntLineWidth(const char *pChars, UInt16 length) SYS_TRAP(sysTrapFntLineWidth);
UInt16 FntWordWrap(const char *chars, UInt16 maxWidth) SYS_TRAP(sysTrapFntWordWrap);
void FntGetScrollValues(const char *chars, UInt16 width, UInt16 scrollPos, UInt16 *linesP, UInt16 *topLine) SYS_TRAP(sysTrapFntGetScrollValues);
Err FntDefineFont(FontID font, FontPtr fontP) SYS_TRAP(sysTrapFntDefineFont);

//jwDone
FontID FontSelect(FontID fontID) SYS_TRAP(sysTrapFontSelect);
