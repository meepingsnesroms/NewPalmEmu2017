 ...
 * 
 * $Id: particles.cpp,v 1.7 2003/04/16 09:35:49 jari Exp $
...
void MyApp::render()
{
    ...
    // Animate (tick) the particle system.
    mPS->tick(XFcCore::getTick());
    // Render the particle system (the above matrices are still in effect).
    mPS->render(mGL);
...
{
    /* // Enabling this code will make the application quit on control key lift.
...
{   
    ...
    mTexture = XFcGLTextureFromFile::create("alphatest.tga");
    ...
    mPS = new XFuParticleSystem();
    XFCASSERT(mPS && "Out of memory allocating PS.");
    mPS->load("bounce.pr3");
    mPS->loadTextures();
    mPS->restart();
    mPS->mSizeScale = 2;
    mPS->mAABB[0] = mPS->mAABB[1] = mPS->mAABB[2] = 5.0f / 1.2f;
}
...
void xfcAppPref(XFcAppPrefs &aAppPrefs)
{
    ...
    aAppPrefs.mUID = 0x101F4F8D; // Symbian application unique ID
...
INT xfcAppInit() 
{
    ...
    //  The xfcUseImageFilterTGA is needed to load the textures. If you disable 
