...
typedef enum {
	...
	sysEventFrmFocusTakeEvent 	= 0x4004,
	sysEventFrmFocusLostEvent 	= 0x4005,
...
struct _KeyUpEventType {
   ...
   UInt16         keyCode;          // virtual key code
   UInt16         modifiers;
   UInt32         msecDown;			// how long the key was down (milliseconds)
   };
...
struct _KeyHoldEventType {
   ...
   UInt16         modifiers;
   UInt32         msecDown;			// how long the key was down (milliseconds)
   UInt16         repeatCount;		// how many times has the event repeated
   };
...
// Implemented in Window.c because of a shared dispatch table for the selector-based trap
void EvtGetPenNative(WinHandle winH, Int16* xP, Int16* yP, Boolean* penDownP);
