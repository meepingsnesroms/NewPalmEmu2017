 ...
 *
 * History:
 *   09/10/00	Created by David Kammer and Eric McManus
 *
 *****************************************************************************/
...
//--------------------------------------------------------------------	
// Used to acquire/release the foreground/background mutex.
//
Err BtLibMutex(UInt16 btLibRefNum, Boolean acquire)
	BTLIB_TRAP(btLibTrapMutex);

//--------------------------------------------------------------------	
// Opens the lib in background mode.
//
Err BtLibOpenBackground( UInt16 btLibRefNum)
	BTLIB_TRAP(btLibTrapOpenBackground);

//--------------------------------------------------------------------	
// Directs library to release all resources and prepare to be removed.
//
Err BtLibUnload(UInt16 btLibRefNum)
	BTLIB_TRAP(btLibTrapUnload);
