...
void dmnewrecord() {
	...
	if(recaddr) {
		...
		if(index >= apps[dmopenref].parts.size() || index == dmMaxRecordIndex) {
			...
			apps[dmopenref].parts[index] = newrec;
		}
...
void dmopendatabaseinfo() {
	...
	if(dbidptr) {
		put_long(dbidptr, dmopenref + dmOpenRefOffset);
	}

	if(cardnumptr) {
		put_word(cardnumptr, 0);
	}

	if(opencountptr) {
		put_word(opencountptr, 1);    //only 1 app is running and it is reading this so always 1
	}

	if(modeptr) {
		put_word(modeptr, apps[dmopenref].openmode);
	}

	if(resdbptr) {
		put_word(resdbptr, apps[dmopenref].resdb);    //maybe uint8_t
	}
...
void dmget1resource() {
	...
	if(!resource) {
		resource = getresource(curapp, id, type);
	}
...
void dmgetresource() {
	...
	for(openappscan = 0; openappscan < totalapps; openappscan++) {
		if(apps[openappscan].open) {
			resource = getresource(openappscan, id, type);
		}
...
void dmdatabaseinfo() {
	...
	if(dbnameptr) {
		writestring(dbnameptr, (char*)apps[dbid].name);
	}
...
void dmsetdatabaseinfo() {
	...
	*/
	...
	if(versionptr) {
		apps[localid].version = get_word(versionptr);
	}

	if(crdateptr) {
		apps[localid].creationtime = get_long(crdateptr);
	}

	if(moddateptr) {
		apps[localid].modificationtime = get_long(moddateptr);
	}

	if(lstbkpdateptr) {
		apps[localid].backuptime = get_long(lstbkpdateptr);
	}

	if(modnumptr) {
		apps[localid].modnum = get_long(modnumptr);
	}

	if(appinfoidptr) {
		apps[localid].appinfo = get_long(appinfoidptr);
	}

	if(sortinfoidptr) {
		apps[localid].sortinfo = get_long(sortinfoidptr);
	}

	if(typeptr) {
		apps[localid].type.typen = belong(get_long(typeptr));
	}

	if(creatorptr) {
		apps[localid].creator.typen = belong(get_long(creatorptr));
	}
...
void dmfinddatabase() {
	...
	if(database != 0xFFFFFFFF) {
		D0 = database + dmOpenRefOffset;
	}
	else {
		D0 = 0;
	}
...
void dmsearchresource() {
	...
	if(resptr) {
		...
		getnumfromptr(resptr, &app_out, &id_out);

		//no need to check for overlay or duplicates since 2 data chunks cant share one pointer
		if(app_out > -1 && apps[app_out].open) {
			put_long(dmopenrefout, app_out + dmOpenRefOffset);
			D0 = id_out;
		}
		else {
			//some apps like to use fixed address of os resources, this causes major problems
			//that pointer does not correspond to a resource
			dbgprintf("Appnum:%d, Isopen:%d, PtrGiven:0x%08x", app_out, id_out, resptr);
...
void memptrheapid() {
	...
	if(testptr >= HEAP && testptr <= HEAPEND) {
		D0 = 0;    /*dynamic heap*/
	}
	else
		if(testptr >= SAVEDATA && testptr <= SAVEDATAEND) {
			D0 = 1;    /*storage heap*/
		}
		else
			if(testptr >= dyn_start && testptr < lcd_start) {
				D0 = 1;    /*storage heap*/
			}
			else {
				palmabrt();    //hack /*unknown heap*/
			}
}
...
void memhandlefree() {
	...
	if(freememaddr(handle)) {
		D0 = errNone;
	}
	else {
		D0 = memErrInvalidParam;
	}
}
...
void memchunknew() {
	...
	if(attr & (memNewChunkFlagAtStart | memNewChunkFlagAtEnd)) {
		palmabrt();    //hack
	}
...
void memcardinfo() {
	...
	if(cardname) {
		writestring(cardname, "4GB Duck Waddler", 32);    //force size to 32
	}

	if(manufname) {
		writestring(manufname, "The Communist Rabbit Soviet", 32);    //force size to 32
	}
...
void prefgetapppreferencesv10() {
	...
	if(resourceptr) {
		memcpy68k(prefptr, resourceptr, size);
	}
