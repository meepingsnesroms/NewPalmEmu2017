...
inline offset_68k get_bmp_data(offset_68k bmpptr) {
	...
	//color table check
	if(bmpflags & bit(14)) {
		data += bitstocolors(get_byte(bmpptr + 8)) * 4 + 2;
	}
...
inline uint8_t get_bmp_bpp(offset_68k bmpptr) {
	if(get_byte(bmpptr + 9) > 0) {
		return get_byte(bmpptr + 8);
	}
...
inline void set_form_num_objects(offset_68k formptr, uint16_t numobjects) {
	put_word(formptr + 62, numobjects);
}
inline uint16_t get_form_num_objects(offset_68k formptr) {
