...
{	UBYTE val = data;
	...
	CFLG=(Shptr->regs).x=val&1; val = ((ULONG)val >> 1) | sign;
	}}
	NFLG = sign != 0;
	ZFLG = val == 0;
...
{{	CPTR dataa = (Shptr->regs).a[srcreg];
...
{
	ULONG srcreg = (LONG)(BYTE)((opcode & 7) >> 0);
{{	CPTR dataa = (Shptr->regs).a[srcreg] + (LONG)(WORD)nextiword();
	WORD data = get_word(dataa);
	VFLG = 0;
{	UWORD val = data;
	ULONG cmask = 0x8000;
