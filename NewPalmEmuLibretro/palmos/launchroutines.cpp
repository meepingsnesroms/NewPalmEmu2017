...
CPTR outa5,outgblstart,outgblend;

//for non aligned ULONG size offsets in compressed globals
ULONG nonalignedlong(CPTR addr){
...
bool full_init(string& palmname, int w, int h){
	...
	//initARM();//arm emulator sets itself up(including reset) when created so this is not needed
...
void full_deinit(){
	...
	memory_deinit();
	delete[] framebuffer;
	//delete[] formattedgfxbuffer;//later
