...
void timgetticks(){
	//HACK //conversion to int and back wastes precision
	//HACK //time doing other things is counted too,so slow operations
	//(such as printf) will cause random speed boosts to catch up
	/*
	totalticks += (chrono::high_resolution_clock::now() - starttime) / palmTicks(1);
	while(partialticks > 1){
		partialticks -= 1;
		totalticks += 1;
	}
	starttime = chrono::high_resolution_clock::now();
	ULONG intticks = totalticks
	D0 = intticks;
	*/

	//HACK fix timer inaccuracy with partial ticks
...
TEMPHACK
void timgetseconds(){
	//get a decent time library
	/*
	time_t now;
	time(&timer);
	struct tm* nowdecomp = localtime(&now);
	uint64 seconds = 0;
	*/
	//palmabrt();//hack
	D0 = 0x72387443;
