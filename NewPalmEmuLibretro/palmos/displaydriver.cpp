...
void bmpcreate(){
	...
	FBWriter clearbitmap(getbmpdata(newbitmap),w,bpp);
	clearbitmap.rect(0,0,w,h,ERASE,0xFFFF/*white*/,0/*corner rounding*/);
...
bool initdisplaydriver(){
	/*
	//initialize display to white
	int16 cntx,cnty;
	inc_for(cnty,LCDH){
		inc_for(cntx,LCDW){
			put_word(lcd_start + cnty * LCDW + cntx,0xFFFF);
		}
	}
	*/
	offset_68k pos;
	size_t_68k total = LCDW + LCDH;
	for(pos = 0;pos < total;pos += 2){
		put_word(lcd_start + pos,0xFFFF);
	}

	//backup frame for when screen popups modify the framebuffer
	spareframe = new UWORD[LCDW * LCDH];

	//HACK clear spareframe if needed
